...
static char *replace_one_sub(char *new_string, const char *sub, int sub_loc, 
    const char *replace)
{
    char *new_s = mx_strnew(mx_strlen(new_string) - mx_strlen(sub)
        + mx_strlen(replace));
    int i = 0;
    int j = 0;
...
int main() {
    char *s = mx_replace_substr("Ururu turu", "ru", "ta"); //returns "McDonuts"
    printf("%s", s);
